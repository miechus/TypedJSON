 ...
 */
export function jsonMember<TFunction extends Function>(
...
export function jsonMember<TFunction extends Function>(
  optionsOrTarget?: IJsonMemberOptions | Object,
  propKey?: string | symbol,
): PropertyDecorator | void {
  if (
    optionsOrTarget instanceof Object &&
    (typeof propKey === "string" || typeof propKey === "symbol")
  ) {
    const target = optionsOrTarget as Object;
    // For error messages.
    const decoratorName = `@jsonMember on ${nameof(target.constructor)}.${
      String(propKey)
    }`;
    ...
    if (isReflectMetadataSupported) {
      const reflectPropCtor = Reflect.getMetadata(
        "design:type",
        target,
        propKey,
      ) as Function;
      ...
      if (!reflectPropCtor) {
        logError(
          `${decoratorName}: could not resolve detected property constructor at runtime. ${MISSING_REFLECT_CONF_MSG}`,
        );
        return;
      }
      ...
      logError(
        `${decoratorName}: ReflectDecorators is required if no 'constructor' option is specified.`,
      );
      return;
    }
  } else {
    ...
    return (target: Object, _propKey: string | symbol) => {
      ...
      let decoratorName = `@jsonMember on ${nameof(target.constructor)}.${
        String(_propKey)
      }`; // For error messages.
      ...
      if (options.hasOwnProperty("constructor")) {
        if (!isValueDefined(options.constructor)) {
          logError(
            `${decoratorName}: cannot resolve specified property constructor at runtime.`,
          );
          return;
        }
        ...
        if (
          ...
          logWarning(
            `${decoratorName}: detected property type does not match 'constructor' option.`,
          );
        }
        ...
        if (isReflectMetadataSupported) {
          ...
          if (!propCtor) {
            logError(
              `${decoratorName}: cannot resolve detected property constructor at runtime.`,
            );
            return;
          }
...
function isSpecialPropertyType(decoratorName: string, propCtor?: Function) {
  if (propCtor === Array) {
    logError(
      `${decoratorName}: property is an Array. Use the jsonArrayMember decorator to` +
  ...
  if (propCtor === Set) {
    logError(
      `${decoratorName}: property is a Set. Use the jsonSetMember decorator to` +
        ` serialize this property.`,
    );
    return true;
  }
  ...
  if (propCtor === Map) {
    logError(
      `${decoratorName}: property is a Map. Use the jsonMapMember decorator to` +
        ` serialize this property.`,
    );
    return true;
  }
