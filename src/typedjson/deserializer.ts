...
import { Constructor, IndexedObject } from "./types.ts";
import { JsonObjectMetadata } from "./metadata.ts";
import { getOptionValue, mergeOptions, OptionsBase } from "./options-base.ts";
...
export class Deserializer<T> {
  ...
  public convertAsObject(
    ...
    if (typeof sourceObject !== "object" || sourceObject === null) {
      this._errorHandler(
        new TypeError(
          `Cannot deserialize ${objectName}: 'sourceObject' must be a defined object.`,
        ),
      );
      return undefined;
    }
    ...
    let sourceObjectMetadata = JsonObjectMetadata.getFromConstructor(
      expectedSelfType,
    );
    ...
    const typeFromTypeHint = this._typeResolver(
      sourceObject,
      knownTypeConstructors,
    );
    ...
    if (typeFromTypeHint) {
      ...
      if (isSubtypeOf(typeFromTypeHint, expectedSelfType)) {
        ...
        sourceObjectMetadata = JsonObjectMetadata.getFromConstructor(
          typeFromTypeHint,
        );
    ...
    if (sourceObjectMetadata && sourceObjectMetadata.isExplicitlyMarked) {
      ...
      sourceMetadata.dataMembers.forEach((objMemberMetadata, propKey) => {
        ...
        const objMemberDebugName = `${
          nameof(sourceMetadata.classType)
        }.${propKey}`;
        const objMemberOptions = mergeOptions(
          classOptions,
          objMemberMetadata.options,
        );
        ...
        if (
          ...
          sourceObjectWithDeserializedProperties[objMemberMetadata.key] =
            revivedValue;
        } else if (objMemberMetadata.isRequired) {
          this._errorHandler(
            new TypeError(`Missing required member '${objMemberDebugName}'.`),
          );
        }
      });
      ...
      if (typeof sourceObjectMetadata.initializerCallback === "function") {
        try {
          ...
          if (!targetObject) {
            ...
            !(targetObject instanceof sourceObjectMetadata.classType)
          ) {
            throw new TypeError(
              `Cannot deserialize ${objectName}:` +
                ...
                `, and '${
                  nameof(targetObject.constructor)
                }' is not a subtype of` +
                ` '${nameof(sourceObjectMetadata.classType)}'`,
            );
          }
      ...
      if (sourceObjectMetadata.onDeserializedMethodName) {
        ...
        if (
          typeof (targetObject as any)[
            sourceObjectMetadata.onDeserializedMethodName
          ] === "function"
        ) {
          (targetObject as any)[
            sourceObjectMetadata.onDeserializedMethodName
          ...
          typeof (targetObject.constructor as any)[
            sourceObjectMetadata.onDeserializedMethodName
          ] === "function"
        ) {
          (targetObject.constructor as any)[
            sourceObjectMetadata.onDeserializedMethodName
          ]();
        } else {
          this._errorHandler(
            new TypeError(
              `onDeserialized callback '${
                nameof(sourceObjectMetadata.classType)
              }.${sourceObjectMetadata.onDeserializedMethodName}' is not a method.`,
            ),
          );
        }
      }
  ...
  public convertSingleValue(
    ...
    let srcTypeNameForDebug = sourceObject
    ...
    if (this.retrievePreserveNull(memberOptions) && sourceObject === null) {
      ...
      if (sourceObject.constructor === expectedSelfType) {
        ...
        throw new TypeError(
          this._makeTypeErrorMessage(
            nameof(expectedSelfType),
      ...
      expectedSelfType === Float32Array || expectedSelfType === Float64Array
    ) {
      ...
      if (Array.isArray(sourceObject)) {
        ...
        this._throwTypeMismatchError(
          "Map",
          "a source array of key-value-pair objects",
          srcTypeNameForDebug,
          memberName,
        );
      }
  ...
  public convertAsArray(
    ...
    if (!typeInfo.elementConstructor || !typeInfo.elementConstructor.length) {
      this._errorHandler(
        new TypeError(
          `Could not deserialize ${memberName} as Array: missing constructor reference of Array elements.`,
        ),
      );
      return [];
    }
    ...
    let elementTypeInfo: IScopeTypeInfo = {
      ...
      elementConstructor: (typeInfo.elementConstructor.length > 1)
        ? typeInfo.elementConstructor.slice(1)
        : [],
  ...
  public convertAsMap(
    ...
    sourceObject.forEach((element: any) => {
      try {
        let key = this.convertSingleValue(
          element.key,
          keyTypeInfo,
          memberName,
          memberOptions,
  ...
  private _convertAsFloatArray<T extends Float32Array | Float64Array>(
    ...
    if (
      Array.isArray(sourceObject) && sourceObject.every((elem) => !isNaN(elem))
    ) {
  ...
  private _convertAsUintArray<
    T extends Uint8Array | Uint8ClampedArray | Uint16Array | Uint32Array,
  >(
    sourceObject: any,
    arrayType: Constructor<T>,
    srcTypeNameForDebug: string,
  ...
  private _makeTypeErrorMessage(
    ...
    const expectedTypeName = (typeof expectedType === "function")
      ? nameof(expectedType)
      : expectedType;
    const actualTypeName = (typeof actualType === "function")
      ? nameof(actualType)
      : actualType;
  ...
  private _createKnownTypesMap(knowTypes: Set<Function>) {
    ...
    knowTypes.forEach((ctor) => {
      if (this._nameResolver) {
        ...
        const name =
          knownTypeMeta && knownTypeMeta.isExplicitlyMarked &&
            knownTypeMeta.name
            ? knownTypeMeta.name
            : ctor.name;
        map.set(name, ctor);
  ...
  private retrievePreserveNull(memberOptions?: OptionsBase): boolean {
    return getOptionValue(
      "preserveNull",
      mergeOptions(this.options, memberOptions),
    );
  }
}
