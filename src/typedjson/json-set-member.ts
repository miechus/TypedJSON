...
export function jsonSetMember(
  ...
  return (target: Object, propKey: string | symbol) => {
    const decoratorName = `@jsonSetMember on ${nameof(target.constructor)}.${
      String(propKey)
    }`; // For error messages.

    if (typeof elementConstructor !== "function") {
      logError(
        `${decoratorName}: could not resolve constructor of set elements at runtime.`,
      );
      return;
    }
    ...
    if (
      ...
      logError(
        `${decoratorName}: property is not a Set. ${MISSING_REFLECT_CONF_MSG}`,
      );
      return;
    }
