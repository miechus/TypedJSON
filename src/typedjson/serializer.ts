...
function isArrayTypeInfo(
  typeInfo: IScopeTypeInfo,
): typeInfo is IScopeArrayTypeInfo {
...
function isSetTypeInfo(
  typeInfo: IScopeTypeInfo,
): typeInfo is IScopeSetTypeInfo {
...
function isMapTypeInfo(
  typeInfo: IScopeTypeInfo,
): typeInfo is IScopeMapTypeInfo {
...
export class Serializer {
  ...
  public convertSingleValue(
    ...
    if (
      this.retrievePreserveNull(memberOptions) && sourceObject === null
    ) {
      return null;
    }
  ...
  public convertAsObject(
    ...
    if (
      sourceObject.constructor !== typeInfo.selfType &&
      sourceObject instanceof typeInfo.selfType
    ) {
    ...
    if (sourceTypeMetadata) {
      if (sourceTypeMetadata.beforeSerializationMethodName) {
        ...
        if (
          typeof (sourceObject as any)[
            sourceTypeMetadata.beforeSerializationMethodName
          ] === "function"
        ) {
          (sourceObject as any)[
            sourceTypeMetadata.beforeSerializationMethodName
          ...
          typeof (sourceObject.constructor as any)[
            sourceTypeMetadata.beforeSerializationMethodName
          ] === "function"
        ) {
          (sourceObject.constructor as any)[
            sourceTypeMetadata.beforeSerializationMethodName
          ]();
        } else {
          this._errorHandler(
            new TypeError(
              `beforeSerialization callback '${
                nameof(sourceTypeMetadata.classType)
              }.${sourceTypeMetadata.beforeSerializationMethodName}' is not a method.`,
            ),
          );
        }
      }
  ...
  public convertAsArray(
    ...
    sourceObject.forEach((element, i) => {
      if (
        !(this.retrievePreserveNull(memberOptions) && element === null) &&
    ...
    const typeInfoForElements: IScopeTypeInfo = {
      ...
      elementTypes: expectedElementType.length > 1
        ? expectedElementType.slice(1)
        : [],
    };
  ...
  public convertAsSet(
    ...
    sourceObject.forEach((element) => {
      let resultElement = this.convertSingleValue(
        element,
        elementTypeInfo,
        memberName,
        memberOptions,
  ...
  public convertAsArrayBuffer(buffer: ArrayBuffer) {
    ...
    return Array.from(new Uint16Array(buffer)).map((charCode) =>
